...
class AuthenticationError(SCMException):
    def __init__(
        ...
        super().__init__(
            message=message, status_code=status.HTTP_401_UNAUTHORIZED, code=code
        )
...
class ForbiddenError(SCMException):
    def __init__(self, message: str = "Forbidden", code: str = "forbidden") -> None:
        super().__init__(
            message=message, status_code=status.HTTP_403_FORBIDDEN, code=code
        )
...
class NotFoundError(SCMException):
    def __init__(
        ...
        super().__init__(
            message=message, status_code=status.HTTP_404_NOT_FOUND, code=code
        )
...
class ConflictError(SCMException):
    def __init__(self, message: str = "Conflict", code: str = "conflict") -> None:
        super().__init__(
            message=message, status_code=status.HTTP_409_CONFLICT, code=code
        )
