...
def get_encryption_key() -> bytes:
    ...
    key_str = getattr(settings, 'field_encryption_key', getattr(settings, 'FIELD_ENCRYPTION_KEY', None))
    if not key_str:
...
def decrypt_field(encrypted_data: bytes, key: bytes) -> str:
    ...
    try:
        ...
        raise ValueError("Falló la verificación de integridad del cifrado. Los datos pueden estar corruptos.")
